...
math <- math[c(3,7,8,13:15,24:31)]
por <- por[c(3,7,8,13:15,24:31)]

math[1:13] = lapply(math[1:13], function(x) as.numeric(x))
por[1:13] = lapply(por[1:13], function(x) as.numeric(x))

math[1:13] = lapply(math[-14], function(x) scale(x))
por[1:13] = lapply(por[-14], function(x) scale(x))
...
math_kNN <- train(G3~., #model formula everything used to classify outcome
                   data = math.training, #use the training data
                   method = 'knn',# indicates the use of kNN model
                   na.action = na.omit)


math_eval <-(predict(math_kNN, newdata = math.test))

math_eval_prob <- predict(math_kNN, newdata = math.test, type = "prob")

math_conf_matrix <- confusionMatrix(math_eval, math.test$G3, positive = "1", dnn=c("Prediction", "Actual"), mode = "everything")

math_conf_matrix

set.seed(2000)
por_kNN <- train(G3~., #model formula everything used to classify outcome
                   data = por.training, #use the training data
                   method = 'knn', #indicates the use of kNN model
                   na.action = na.omit)


por_eval <-(predict(por_kNN, newdata = por.test))

por_eval_prob <- predict(por_kNN, newdata = por.test, type = "prob")

por_conf_matrix <- confusionMatrix(por_eval, por.test$G3, positive = "1", dnn=c("Prediction", "Actual"), mode = "everything")
